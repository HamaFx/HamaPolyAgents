import { fail, ok, parseJson } from "@/app/api/_shared";
import { AnalystAgent } from "@/lib/ai/agents/analyst";
import { ResearcherAgent } from "@/lib/ai/agents/researcher";
import { buildSemanticAnchor } from "@/lib/ai/prompts/semantic-anchor";
import { researcherOutputSchema } from "@/lib/ai/schemas/researcher-output";
import { z } from "zod";

const schema = z.object({
  teamId: z.string().min(1),
  marketId: z.string().min(1),
  research: z.unknown().optional()
});

export async function POST(request: Request) {
  const parsed = await parseJson(request, schema);
  if (!parsed.data) {
    return fail(parsed.error ?? "Invalid payload.");
  }

  try {
    const anchor = buildSemanticAnchor(parsed.data.teamId, parsed.data.marketId);

    let research;
    if (parsed.data.research) {
      research = researcherOutputSchema.parse(parsed.data.research);
    } else {
      const researcher = new ResearcherAgent();
      research = (await researcher.execute(anchor)).output;
    }

    const agent = new AnalystAgent();
    const result = await agent.execute({ anchor, research });
    return ok(result);
  } catch (error) {
    return fail(error instanceof Error ? error.message : "Analyst failed.", 422);
  }
}
